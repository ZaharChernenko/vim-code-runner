...
def test_shebang_command_builder_empty(fixture_shebang_command_builders_dispatcher, content):
    if content is None:
...
def test_get_shebang(fixture_shebang_command_builders_dispatcher, content, expected_shebang):
    with tempfile.NamedTemporaryFile(mode="w", delete=False) as temp_file:
        temp_file.write(content)
        file_path = temp_file.name
    try:
        builder: Optional[TShebangCommandBuilder] = fixture_shebang_command_builders_dispatcher.dispatch(file_path)
        assert builder is not None
        assert builder.build(file_path) == f"{expected_shebang} '{file_path}'"
    finally:
        os.unlink(file_path)
